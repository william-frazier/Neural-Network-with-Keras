...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        for i in range(1, n): # 1 .. n-1
            b = self.biases[i-1]
            w = self.weights[i-1]
            z[i] = np.dot(w, np.float_(a[i-1])) + b
            a[i] = sigmoid(z[i])
        ...
        delta[i] = (a[i] - np.int_(y)) * sigmoid_grad(z[i])
        ...
        for i in range(0, n-1):
            grad_b[i] = delta[i+1]
            grad_w[i] = np.dot(delta[i+1], np.float_(a[i].T))
    ...
    def report_accuracy(self, epoch, train_data, valid_data):
        ...
        print("train %d/%d (= %.2f) " % (ntr, tr, tr/ntr*100), end='')
        print("valid %d/%d (= %.2f) " % (nte, te, te/nte*100))
